...
template_values(app.config['AUTH_TOKEN'], POD_IP, file_data, base_cpu + 0.2, cr, m, mr, replica_calculated, ((" " + " ".join(sys.argv[2:])) if len(sys.argv) > 2 and sys.argv[2] != "run" else ((" " + " ".join(sys.argv[4:]) if len(sys.argv) > 2 and sys.argv[2] == "run" else ""))), custom_script_file_data)
