...
RUN apt update
RUN DEBIAN_FRONTEND=noninteractive apt install wget xz-utils iputils-ping netcat-traditional curl gnupg2 zip unzip build-essential vim rsyslog gfortran qtbase5-dev qt5-qmake qtscript5-dev libqt5svg5* libboost-dev libmkl-interface-dev libmkl-computational-dev libmkl-threading-dev -y
...
COPY --from=builder /build/main /bin/orca-executor
